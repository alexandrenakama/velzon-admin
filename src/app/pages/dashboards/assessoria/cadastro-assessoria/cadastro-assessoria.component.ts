// src/app/pages/dashboards/assessoria/cadastro-assessoria/cadastro-assessoria.component.ts
...
import {
  ...
  Contato,
  PessoaTipo
} from 'src/app/store/Assessoria/assessoria.model';
...
@Component({
  selector: 'app-cadastro-assessoria',
  templateUrl: './cadastro-assessoria.component.html',
})
export class CadastroAssessoriaComponent implements OnInit {
  form!: FormGroup;
  isEdit = false;
  submitted = false;
  ...
  ngOnInit(): void {
    ...
    this.route.paramMap.subscribe(params => {
      ...
      const asy = this.service.getById(this.editingId);
      if (!asy) return;

      this.form.patchValue({
        id:         asy.id,
        nome:       asy.nome,
        tipoPessoa: asy.tipoPessoa,
        ativo:      asy.ativo,
        documento:  asy.documento
      });
      this.form.get('id')?.disable();

      this.enderecos.clear();
      asy.enderecos.forEach(e => this.enderecos.push(this.fb.group(e)));

      this.contatos.clear();
      asy.contatos.forEach(c => this.contatos.push(this.fb.group(c)));
    });
  }
  ...
  private uniqueIdValidator(c: AbstractControl): ValidationErrors | null {
    ...
    if (existing && (!this.isEdit || existing.id !== this.editingId)) {
      return { uniqueId: 'Já existe uma assessoria com este ID.' };
    }
    return null;
  }
  ...
  save(): void {
    ...
    const op$ = this.isEdit
      ? this.service.update(asy)
      : this.service.create(asy);

    op$.subscribe({
      next: () => {
        this.toast.show(
          this.isEdit
            ? `Assessoria "${asy.nome}" atualizada!`
            : `Assessoria "${asy.nome}" criada!`,
          {
            classname: this.isEdit ? 'bg-info text-light' : 'bg-success text-light',
            delay: 5000
          }
        );
        if (!this.isEdit) this.router.navigate(['/assessoria']);
      },
      error: () => {
        this.toast.show('Erro ao salvar assessoria.', {
  ...
  cancel(): void {
    this.router.navigate(['/assessoria']);
  }
}
